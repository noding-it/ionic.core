...
export class WordpressService {
  ...
  public getUserOrders(): Observable<any> {
    return this._http.get(
      ...
      {
        headers: new HttpHeaders().set('content-type', 'application/x-www-form-urlencoded').set('Authorization', `Bearer ${localStorage.getItem('token')}`).set('Woocommerce', `${this._viewConfig.environment.WOOCOMMERCE_TOKEN}`)
      }
    ).pipe(
      catchError(this._gs.errorHandler)
    );
  ...
  public createUser(params: object): Observable<any> {
    return this._http.post(
      ...
      {params},
      {
        headers: new HttpHeaders().set('content-type', 'application/json').set('Authorization', `Bearer ${localStorage.getItem('token')}`).set('Woocommerce', `${this._viewConfig.environment.WOOCOMMERCE_TOKEN}`),
      }
    ).pipe(
      catchError(this._gs.errorHandler)
    );
