...
import {ModalPreviewComponent} from '../components/modal-preview.component';
...
export class DropboxService {
    ...
    public supportedExtensionPreview = ['pdf', 'png', 'jpg', 'jpeg', 'gif', 'jfif'];
    ...
    /*public upload(object): Observable<any> {
        return this._gs.callDropbox(object, true);
    }*/

    public get(object): Observable<any> {
        return this._http.get<IGatewayResponse>(
            `${environment.apiGateway}/storage/get/${object.path}`,
            {
    ...
    public delete(object): Observable<any> {
        return this._http.delete<IGatewayResponse>(
            `${environment.apiGateway}/storage/delete/${object.path}`,
            {
                headers: new HttpHeaders().set('content-type', 'application/x-www-form-urlencoded').set('authorization', `UpThere ${localStorage.getItem('token')}`)
            });
    }
