...
export class BrowserService {
   ...
   */
  public getAppVersion(app: string, link = 'https://chrome.google.com/webstore/detail/myvirtualab-chromium-tool/mgflnancoccobpfodoimfkihhihlhlni?hl=it&authuser=0'): void {
    if (!isDevMode()) {
      let currentVersion: string = '';
      if (localStorage.getItem('mvl')) {
        currentVersion = JSON.parse(localStorage.getItem('mvl') as string).version;
      }
